...
type Box<T> = T | [T] | [T, T] | [T, T, T] | [T, T, T, T]
...
type Repeat = 'repeat-x' | 'repeat-y' | 'repeat' | 'no-repeat'
...
type Origin = 'margin' | 'border' | 'padding' | 'content'
...
type Radius = Length | [Length] | [Length, Length]
...
export function create<
  T extends { [key: string]: Partial<Styles> },
  ...
  Object.keys(arg).forEach(k => {
    ...
    result[k] =
      Object.entries(stylesheet)
        .map(([k, v]) =>
          wrapEntry(k as keyof Styles, v as Styles[keyof Styles])
        )
        .join(';') + ';'
  })
